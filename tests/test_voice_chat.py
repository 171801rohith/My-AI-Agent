...
def test_termination_phrases(voice_chat, phrase):
    assert voice_chat.check_for_termination(phrase.split(" "))


@pytest.mark.parametrize("phrase", ["chat", "open chat", "exit", "chat exit"])
def test_non_termination_phrases(voice_chat, phrase):
    assert not voice_chat.check_for_termination(phrase.split(" "))


@pytest.mark.xfail(reason="STT output carries punctuation ('Exit chat.') which is not stripped")
def test_termination_with_punctuation(voice_chat):
    assert voice_chat.check_for_termination("exit chat.".split(" "))
